                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('user.update',$user->id) }}" enctype="multipart/form-data">
                            ...
                            {{-- profile image --}}
                            <div class="form-group row">
                                <label for="avatar" class="col-md-4 col-form-label text-md-center">{{ __('users.avatar') }}</label>

                                <div class="col-md-6">
                                    <input id="avatar" type="file" class="form-control @error('avatar') is-invalid @enderror" name="avatar" accept="image/*">
                                    @if ($user->avatar)
                                        <div class="mt-2">
                                            <img src="{{ asset($user->avatar) }}" alt="{{ $user->name }}" class="img-thumbnail" style="width: 100px;height: 100px;border-radius: 50%;">
                                        </div>
                                    @endif
                                    @error('avatar')
                                    <span class="invalid-feedback" role="alert">
